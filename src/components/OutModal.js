...
export default props => {
    ...
    const { dark, addOut, outModal, setOutModal } = useContext(Context)
    ...
    return(
        <Modal
            ...
            visible={outModal}
            transparent
            onRequestClose={() => setOutModal(!outModal)}>
                <View style={styles.container}>
                    <View style={[styles.modal, {backgroundColor: dark ? '#4A4443' : '#fff', }]}>
                        <View style={styles.header}>
                            <Text style={[styles.title, {color: dark ? '#fff' : '#000'}]}>Cadastrar Saída</Text>
                        </View>
                        <View>
