...
export default props => {
    ...
    const { dark, addFixed, fixedModal, setFixedModal } = useContext(Context)
    ...
    return(
        <Modal
            ...
            visible={fixedModal}
            transparent
            onRequestClose={() => setFixedModal(!fixedModal)}>
                <View style={styles.container}>
                    <View style={[styles.modal, {backgroundColor: dark ? '#4A4443' : '#fff', }]}>
                        <View style={styles.header}>
                            <Text style={[styles.title, {color: dark ? '#fff' : '#000'}]}>Cadastrar Gasto Fixo</Text>
                        </View>
                        <View>
